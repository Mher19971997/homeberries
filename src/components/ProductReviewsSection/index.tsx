...
import {
  ...
  StarEmptyIcon,
  ChevronDownIcon,
} from "@homeberris/assets/icons/reviews";
...
function CommentInput({
  ...
  return (
    <div
      style={{
        marginTop: 20,
        border: `1px solid ${focused ? "#242424" : "#d5d5d5"}`,
        borderRadius: 8,
      ...
      {focused && (
        ...
        >
          ...
          <span
            ...
            title={t("productReviews.attachFile")}
          >
            <Paperclip size={16} strokeWidth={1.8} />
            {!image && (
              <span style={{ fontSize: 12 }}>
                {t("productReviews.attachFile")}
              </span>
            )}
          </span>
          <input
          ...
          />
          {image && (
            <span style={{ fontSize: 12, color: "#868695" }}>{image.name}</span>
          )}
          {/* Send */}
          <button
            onClick={onSend}
...
export default function ProductReviewsSection({ catalog }: Props) {
  ...
  const avgRating =
    comments.length > 0
      ? Math.round(
          (comments.reduce((sum: number, c: any) => sum + (c.rating || 0), 0) /
            comments.length) *
            10,
        ) / 10
      : 0;
  ...
  const bars = [
    ...
    label: b.label,
    count: comments.filter((c: any) => c.rating >= b.min && c.rating <= b.max)
      .length,
  }));
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () =>
      createComment(
        {
          text,
          catalogUuid: catalog?.uuid as UUID,
          rating,
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["getCatalogByUud", catalog?.uuid],
      });
    },
  });
