...
export const OrderCard = React.memo(({ order, onMenuOpen, onPay }: Props) => {
  ...
  const getStatusLabel = (s: string) => {
    switch (s) {
      ...
      case "cancelled":
      case "canceled":
        return t("delivery.status.cancelled");
      case "processing":
      case "in_progress":
        return t("delivery.status.processing");
      case "shipped":
        return t("delivery.status.shipped");
      case "pending":
        return t("delivery.status.pending");
      case "paid":
        return t("delivery.status.paid");
      default:
        return s || t("delivery.status.unknown");
    }
  };
  ...
  return (
    <div className={styles.orderCard}>
      <div className={styles.orderHeader}>
        <div>
          <span className={styles.orderMeta}>
            {t("delivery.card.orderNum")}
            {order.uuid.substring(0, 8).toUpperCase()}
          </span>
          <span className={styles.orderMeta} style={{ marginLeft: 16 }}>
            {formatDateLocal(order.createdAt)}
          </span>
        </div>
        <div className={styles.orderHeaderRight}>
          <span
            className={`${styles.statusChip} ${styles[`status_${status}`] || styles.status_default}`}
          >
      ...
      <p className={styles.orderQty}>
        {t("delivery.card.qty")} {order.quantity}
      </p>
